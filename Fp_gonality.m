...
function DivisorsCandidatesByPartition(degree_counts, degree_partition, n)
...
function DivisorsCandidates(degree_counts, n, cache, filter : First := false)
...
//todo optimize so that we only loop over degree_partitions so that degree_partitions[1] is at least #X(Fq) / (q+1)
    d := #degree_counts;
    divisors := [];
    for degree_partition in IntegerSolutions([1..Min(d-n,1)],d-n) do
        degree_partition[1] +:= n;
        for D in DivisorsCandidatesByPartition(degree_counts, degree_partition, n) do
...
function HasFunctionOfDegreeAtMost(C, d)
    ...
    g_d_1s := DivisorsCandidates(degree_counts, n, powerseries_expansions, HasNonconstantFunction : First := true);
    return #g_d_1s ge 1;
end function;
